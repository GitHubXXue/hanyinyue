$(function(){
	...
	var liNum = demoArr.length; // 暂且认为li个数为 5*5*5 个
	...
	function init(){
		...
		{
			var $li = $('<li><p class="title">'+demoArr[i].title+'</p><p class="author">'+demoArr[i].author+'</p><p class="time">'+demoArr[i].date+'</p></li>');
	...
	(function(){
		...
		var $show = $('#show');
		for(i=0;i<liNum;i++){
			$mainLi[i].index=i;//自定义属性
		}
		$mainLi.click(function(ev){
			ev = ev || window.event;
			document.querySelector("#show .s-title").innerHTML = demoArr[this.index].kt;
			document.querySelector("#show .s-img img").src = demoArr[this.index].src;
			document.querySelector("#show .s-author").innerHTML = "歌手:"+demoArr[this.index].author;
			document.querySelector("#show .s-dec").innerHTML = "详情:"+demoArr[this.index].ms;
		...
		$show.click(function(ev){
			...
			$('#frame').show().animate({
				left : 0
			},1000).find('iframe').attr('src' , '../index.html');
			ev.stopPropagation();
		});
