...
//先初始化一首歌
    function searchMusic(songname,page) {
        $Tbody.html("");//清空
        var url = "https://route.showapi.com/213-1?keyword="+songname+"&page="+page+"&showapi_appid=62322&showapi_test_draft=false&showapi_timestamp="+getTime()+"&showapi_sign=c5175828b1bb4f7fa419a342233eb7f3"
...
//点击播放
    $Tbody.on("dblclick","tr",function () {//双击事件，tr确定事件源

    //  播放当前点击的歌曲
        //$Audio.prop("src","$(this).data("m4a")")[0].play();//.play原生audio下面属性，jq是调用不了的,[0]变为原生对象
        $Audio.prop("src","http://ws.stream.qqmusic.qq.com/C400003GBUdq4W6cOD.m4a?vkey= &guid= &uin= &fromtag=0")[0].play();
        $Play.find(".play").addClass("zanting").removeClass("bofang");//初始都是暂停
        $Audio.data({
            "showTime": $(this).index(),//当前播放序号
            "smallImg" : $(this).data("albumpicsmall"),
            "bigImg" : $(this).data("albumpicsbig"),
...
// input搜索歌曲
    $("#header .search").keyup(function (e) {
        ...
        var val = $(this).val();
        if(val){
            $Tbody.data("songname",val)//绑定歌曲名
            searchMusic(val,1)//默认请求第一页
            closeLayer();
        }
    });
//点击播放暂停
    (function () {
        var $sidebarFooter = $(".sidebarFooter"),
            $mainBigImg = $(".mainBigImg"),
            $smallImg = $sidebarFooter.children("img"),
            $bigImg = $mainBigImg.children("img"),
        ...
        $player.on("click",function () {
            if($(this).hasClass("bofang")){//真就是没有播放音乐.hasClass判断对象的类名
                $Audio[0].play()//原生的播放空间按钮
            }else{//正在播放音乐，点击暂停
                $Audio[0].pause()//.pause暂停
            }
            $(this).toggleClass("zanting bofang")//有暂停，没有播放
        ...
        $prev.on("click",function () {
            ...
            var $TrThis = $Tr.eq(showTime);
               ...
               //$Audio.prop("src",$TrThis.data("m4a"))[0].play(0)//换完地址让它播放
               $Audio.prop("src","http://ws.stream.qqmusic.qq.com/C400003GBUdq4W6cOD.m4a?vkey= &guid= &uin= &fromtag=0")[0].play();
            $Audio.data({
                showTime : showTime,
        ...
        $next.on("click",function () {
            ...
            $Audio.data({
                ...
                singername : $TrThis.data("singername"),
                smallImg : $TrThis.data("albumpicsmall"),
                bigImg : $TrThis.data("albumpicsbig"),
                "songid" : $(this).data("songid")
            });
        });
    })();
//同步进度条
    function Synchronize() {
        ...
        var $TotalTime = $Audio[0].duration,
            currentTime = $Audio[0].currentTime;//当前时间
    ...
    (function(){
        $(".progressBar .bar").on("mousedown",function (e) {
            ...
            $(document).on("mousemove",function (e) {
                ...
                _X = Math.min(_X,max);
                OldWidth = _X;
                $target.prev().css("width",_X)//前一个对象的宽度
...
//当资源刚加载开始：
    $Audio.on("loadstart",function () {
        ...
        $bigImg.prop("src",$(this).data("bigImg"));
        //  歌名
    ...
    function getLyric(songid) {
        ...
        $.getJSON(url,function (msg) {
            ...
            $("#FMlyric").html().replace(/\[([\d:.]+)\](.+)/g,function(a,$1,$2){
                $("<li></li>").data("time",$1).html($2).appendTo($("#lyricList"))//li对象定义一个自定义属性，
